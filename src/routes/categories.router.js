...
export default class CategoriesRouter extends Router {
    init() {
        this.post("/", [accessRolesEnum.PUBLIC], passportStrategiesEnum.NOTHING, uploader.single("image"), async (req, res) => {
            try {
                ...
                const category = new categoriesModel({ name, parent: parentId, image: imagePath });
                await category.save();
                res.json(category);
            } catch (error) {
        ...
        // Editar categoría
        this.put("/:id", [accessRolesEnum.PUBLIC], passportStrategiesEnum.NOTHING, uploader.single("image"), async (req, res) => {
        try {
            const { id } = req.params;
            const { name, parent } = req.body;
